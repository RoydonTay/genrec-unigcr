...
from genrec.models.tiger_augr import TigerWithRanking
...
def train(
    ...
    use_direct_catalog_loss=False,
    ctr_hidden_units=(256, 128),
):
    """
    ...
    model = TigerWithRanking(
        ...
        use_direct_catalog_loss=use_direct_catalog_loss,
        catalog_mlp_hidden_units=ctr_hidden_units,
    )
    ...
    if hasattr(model, "valid_item_ids"):
        model.valid_item_ids = valid_item_ids
    num_catalog_items = valid_item_ids.size(0)

    def _encode_sid(ids: torch.Tensor, base: int) -> torch.Tensor:
        enc = torch.zeros(ids.size(0), device=ids.device, dtype=torch.long)
        for step in range(ids.size(1)):
            enc = enc * base + ids[:, step]
        return enc

    catalog_enc = _encode_sid(valid_item_ids, num_item_embeddings)

    def _target_item_indices(target_sids: torch.Tensor) -> torch.Tensor:
        target_enc = _encode_sid(target_sids, num_item_embeddings)
        match = catalog_enc.unsqueeze(0) == target_enc.unsqueeze(1)
        if not match.any(dim=1).all():
            raise ValueError("Some target_input_ids are not present in valid_item_ids.")
        return match.float().argmax(dim=1)
            ...
            with accelerator.accumulate(model):
                with accelerator.autocast():
                    output = model(
                        ...
                        valid_item_ids=valid_item_ids,
                        target_item_indices=(
                            _target_item_indices(data["target_input_ids"].to(device))
                            if use_direct_catalog_loss
                            else None
                        ),
                        num_catalog_items=num_catalog_items if use_direct_catalog_loss else None,
                    )
                    loss = output.loss
    ...
    # Final evaluation on best checkpoint (if present)
    if accelerator.is_main_process:
        ...
        if os.path.exists(best_path):
            ...
            logger.info(f"Loaded best model for final test eval: {best_path}")

    accelerator.wait_for_everyone()
    if accelerator.is_main_process:
        best_test_metrics = evaluate(test_dataloader, desc="Final Test (Best Model)")
        logger.info(f"Final Test (Best Model): {best_test_metrics}")
        if wandb_logging:
            wandb.log({f"eval/best_{k}": v for k, v in best_test_metrics.items()})
