...
class CTR(nn.Module):
    """
    CTR model for sequential recommendation.
    ...
    def __init__(
        ...
        Args:
            ...
            use_dot_product_logits: Whether to use dot product for logits computation
        """
        super().__init__()
    ...
    def forward(
        ...
        if self.use_dot_product_logits:
            # Compute logits as dot product with item embeddings
        ...
        if targets is not None:
            ...
            flat_logits = ctr_logits.view(-1, self.num_items + 1) # [B*L, V]
            flat_relevance = relevance.view(-1, self.num_items + 1) # [B*L, V]
